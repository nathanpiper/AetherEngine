...
| Disc        | DVD-Video ISO (decrypted): reads the ISO9660 bridge filesystem, selects the longest title set, and demuxes its concatenated VOBs as MPEG-PS through the normal decode path. No decryption (CSS retail discs must be ripped decrypted first), no menus / multi-angle, main title only. Blu-ray ISO not yet supported |
